...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, addTimeStamp) {
    ...
    var txTimestamp = reward === 'POS' ?
        ...
        addTimeStamp ? util.packUInt32LE(rpcData.curtime) :
        new Buffer([]);

    //For coins that support/require transaction comments
    var txComment = txMessages === true ?
        util.serializeString('http://bitchk.com') :
        new Buffer([]);
    ...
    var scriptSigPart2 = util.serializeString('/bitchk/');
